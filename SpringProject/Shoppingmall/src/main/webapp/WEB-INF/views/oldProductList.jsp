				...
				<!-- Main -->
					<div id="main">
						...
						<c:forEach items="${productList}" var="product">
								<article>
										<header>
											<h2>[중고]<a href="/shoppingmall/productDetail?pId=${product.pId}">${product.pName }</a></h2>
										</header>
							...
							<c:if test="${pageDto.totalCount gt 0}">
								<%-- [이전] 출력 --%>
									<c:if test="${pageDto.startPage gt pageDto.pageBlock}">
										<a class="previous" href="oldProductList?strPageNum=${pageDto.startPage - pageDto.pageBlock}&field=${field}&word=${word}">Prev</a>
									</c:if>
									...
									<c:forEach var="i" begin="${pageDto.startPage}" end="${pageDto.endPage}" step="1">
										<c:choose>
											<c:when test="${pageScope.i eq requestScope.pageNum}">
												<a class="page" href="/shoppingmall/oldProductList?strPageNum=${i}&field=${field}&word=${word}">${i}</a>
											</c:when>
											<c:otherwise>
												<a class="page" href="/shoppingmall/oldProductList?strPageNum=${i}&field=${field}&word=${word}">${i}</a>
											</c:otherwise>
								...
								<%-- [다음] 출력 --%>
									<c:if test="${ pageDto.endPage lt pageDto.pageCount}">
										<a class="next" href="/shoppingmall/oldProductList?strPageNum=${pageDto.startPage + pageDto.pageBlock}&field=${field}&word=${word}">Next</a>
									</c:if>
							</c:if>
							</div>
